...
(define (test-cbc-multi-blocks name scheme key iv pt ct)
  ...
  (let ((bv (symmetric-cipher:encrypt-bytevector cipher skey param pt)))
    ...
    (test-equal (string-append name " dec") pt
		(symmetric-cipher:decrypt-bytevector cipher skey param bv))))
...
(for-each (lambda (e)
	    ...
	    (let ((name (string-append "AES 128 (" (ref 'COUNT e) ")"))
	      ...
	      (test-cbc-multi-blocks name *scheme:aes-128* key iv pt ct)))
	  aes-test-vectors)
